...
class ForumService:
    ...
    def create_thread(self, *, author_user_id: int, title: str, content: str, category: str, lease_order_id: Optional[int]) -> ForumThread:
        ...
        if lease_order_id:
            ...
            if not exists:
                lease_order_id = None
        th = ForumThread(title=title, author_user_id=author_user_id, category=category, lease_order_id=lease_order_id)
        self.db.add(th)
        self.db.flush()
        post = ForumPost(thread_id=th.id, author_user_id=author_user_id, content=content)
        self.db.add(post)
        self.db.commit()
        return th
    ...
    def create_post(self, *, thread_id: int, author_user_id: int, content: str, parent_post_id: Optional[int]) -> ForumPost:
        th = self.get_thread(thread_id)
        post = ForumPost(thread_id=th.id, author_user_id=author_user_id, content=content, parent_post_id=parent_post_id)
        self.db.add(post)
        self.db.commit()
        return post
